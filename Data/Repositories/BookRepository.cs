...
namespace Biblioteka.Data.Repositories
{
    public class BookRepository : BaseRepository
    {
        public void Add(Book book)
        {
            ...
            cmd.CommandText = @"INSERT INTO books (title,author,isbn,publisher,year,pages,genre,language)
                                VALUES (@title,@author,@isbn,@publisher,@year,@pages,@genre,@language);";
        ...
        public Book GetById(int id)
        {
            ...
            conn.Open();
            using var cmd = conn.CreateCommand();
            cmd.CommandText = @"SELECT id,title,author,isbn,publisher,year,pages,genre,language
        ...
        public IEnumerable<Book> GetAll()
        {
            ...
            conn.Open();
            using var cmd = conn.CreateCommand();
            cmd.CommandText = @"SELECT id,title,author,isbn,publisher,year,pages,genre,language FROM books;";
            using var reader = cmd.ExecuteReader();
            while (reader.Read())
            {
                list.Add(new Book
        ...
        public void Update(Book book)
        {
            ...
            conn.Open();
            using var cmd = conn.CreateCommand();
            cmd.CommandText = @"UPDATE books SET title=@title,author=@author,isbn=@isbn,publisher=@publisher,
                                year=@year,pages=@pages,genre=@genre,language=@language WHERE id=@id;";
            cmd.Parameters.AddWithValue("@title", book.Title);
            cmd.Parameters.AddWithValue("@author", book.Author);
            cmd.Parameters.AddWithValue("@isbn", book.ISBN);
            cmd.Parameters.AddWithValue("@publisher", (object)book.Publisher ?? DBNull.Value);
        ...
        // Additional search method by ISBN
        public Book GetByISBN(string isbn)
        {
            using var conn = new SQLiteConnection(_connectionString);
            conn.Open();
            using var cmd = conn.CreateCommand();
            cmd.CommandText = "SELECT id,title,author,isbn,publisher,year,pages,genre,language FROM books WHERE isbn=@isbn;";
